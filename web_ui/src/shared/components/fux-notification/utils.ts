...
interface FuxNotificationData {
    ...
    description: string;
    docUrl: DocsUrl | string | undefined;
...
export const getFuxNotificationData = (fuxNotificationId: string): FuxNotificationData => {
    ...
    const isAdmin = useCheckPermission(
        ...
        [
            {
                type: RESOURCE_TYPE.ORGANIZATION,
                id: 'admin',
            },
        ]
    );
    ...
    switch (fuxNotificationId) {
        ...
        case FUX_NOTIFICATION_KEYS.AUTO_TRAINING_NOTIFICATION:
            return {
                header: '',
                description: '',
                docUrl: isAdmin ? paths.account.usage({ organizationId }) : undefined,
                nextStepId: undefined,
                previousStepId: undefined,
                showDismissAll: false,
                tipPosition: TipPosition.TOP_RIGHT,
