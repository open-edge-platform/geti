...
PRETRAINED_MODELS_LIST_FILEPATH = "pretrained_models.json"
PRETRAINED_MODELS_LIST_FILEPATH_FOR_V2 = "pretrained_models_v2.json"
...
def get_file(url: str, target_path: str, auto_unzip=True, local_mirror=None):  # noqa: ANN001, ANN201, D103
    target_url = url
    if local_mirror is not None:
        target_url = os.path.join(local_mirror, os.path.basename(url))
    try:
...
def download_pretrained_model(model_spec: dict, target_dir: str, local_mirror: str):  # noqa: ANN201, D103
    model_local_mirror_url = local_mirror + os.path.dirname(model_spec["local_mirror"])
    ...
    try:
        retry_call(
            ...
            local_mirror=model_local_mirror_url,
...
def download_models(models_spec: dict, local_mirror: str):  # noqa: ANN201, D103
    os.makedirs(DOWNLOADED_MODELS_DIR, exist_ok=True)

    for model in models_spec:
        download_pretrained_model(model, DOWNLOADED_MODELS_DIR, local_mirror)


def put_models(models_spec: dict):  # noqa: ANN201, D103
    for model in models_spec:
        target_path = model["target"]

        os.makedirs(os.path.dirname(target_path), exist_ok=True)
        downloaded_model_path = os.path.join(DOWNLOADED_MODELS_DIR, os.path.basename(target_path))
        logger.info(f"Copying model from {downloaded_model_path} to {target_path}")
...
def main():  # noqa: ANN201, D103
    ...
    parser.add_argument("--local-mirror", type=str)
    args = parser.parse_args()
    command = args.command
    local_mirror = args.local_mirror

    with open(PRETRAINED_MODELS_LIST_FILEPATH) as file:
        models_spec_v1 = json.load(file)
    with open(PRETRAINED_MODELS_LIST_FILEPATH_FOR_V2) as file:
        models_spec = json.load(file)

    if command == "download":
        download_models(models_spec, local_mirror=local_mirror)
        # copy the model list metadata file as well
        shutil.copy(
            PRETRAINED_MODELS_LIST_FILEPATH_FOR_V2, os.path.join(DOWNLOADED_MODELS_DIR, PRETRAINED_MODELS_LIST_FILEPATH)
        )
    elif command == "download_v1":
        download_models(models_spec_v1, local_mirror=local_mirror)
        # copy the model list metadata file as well
        shutil.copy(
            PRETRAINED_MODELS_LIST_FILEPATH, os.path.join(DOWNLOADED_MODELS_DIR, PRETRAINED_MODELS_LIST_FILEPATH)
        )
    elif command == "put":
        put_models(models_spec)
    elif command == "put_v1":
        put_models(models_spec_v1)
    else:
