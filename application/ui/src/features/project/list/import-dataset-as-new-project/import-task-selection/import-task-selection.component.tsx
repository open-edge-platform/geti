    ...
    const items = allowedTaskTypes.map((taskType) => ({
        ...
        label:
            defaultTaskType === taskType
                ? `${getTaskTypeTitle(taskType)} ${t('projectList.recommendedSuffix')}`
        ...
        <View backgroundColor={'gray-75'} margin={'size-300'} padding={'size-300'}>
            <Form id={TASK_SELECTION_FORM_ID} validationBehavior='native' action={submitAction}>
                <TextField
                    ...
                    aria-label={t('projectForm.nameLabel')}
                ...
                <Picker
                    ...
                    items={items}
                    name={'task_type'}
                    label={t('projectForm.taskTypeLabel')}
                    aria-label={t('projectForm.taskTypeLabel')}
                    marginBottom={'size-150'}
                    placeholder={t('projectForm.selectTaskPlaceholder')}
                ...
                <View>
                    {defaultTaskType !== undefined && (
                        <Flex gap='size-100' alignItems={'center'}>
                            ...
                            <Text>{t('projectList.importRecommendationInfo')}</Text>
                        </Flex>
                    )}
                </View>
            </Form>
