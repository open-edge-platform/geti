...
class TestGetiTuneTrainerEvaluateModel:
    ...
    def test_evaluate_model_reuses_pytorch_results_when_openvino_construction_fails(
        self,
        fxt_getitune_trainer: Callable[[], GetiTuneTrainer],
        tmp_path: Path,
        fxt_model_service: Mock,
    ):
        """OVEngine construction failures must not fail the job; PyTorch results are reused instead.

        This covers a distinct failure mode from `..._when_openvino_fails`: the exception is
        raised while building the OVEngine itself (e.g. an unsupported ONNX graph rejected by
        OpenVINO's frontend during `OVEngine.__init__`), not while calling `.test()` on an
        ...
        # Act: OVEngine(...) itself raises for the OpenVINO variant (construction failure,
        # not a `.test()` failure), then succeeds for the ONNX variant.
        with (
            patch(
                "getitune.backend.openvino.engine.OVEngine",
                side_effect=[RuntimeError("Model wasn't fully converted"), mock_onnx_engine],
            ),
            patch("app.execution.training.getitune_trainer.logger") as mock_logger,
        ):
            getitune_trainer.evaluate_model(
        ...
        # Assert: a warning was logged for the failed OpenVINO construction
        mock_logger.warning.assert_called_once()
        assert "openvino" in mock_logger.warning.call_args.args[1].lower()
