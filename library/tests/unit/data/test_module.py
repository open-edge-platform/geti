...
class TestDatasetNeedsSingleProcessLoading:
    """Unit tests for `_dataset_needs_single_process_loading`."""

    def test_detects_function_closure_in_object_column(self) -> None:
        import polars as pl
        ...
        dataframe = pl.DataFrame({"mask": pl.Series([_closure], dtype=pl.Object)})
    ...
    def test_no_object_columns_returns_false(self) -> None:
        ...
        dataframe = pl.DataFrame({"labels": [[1, 2]], "image_id": [1]})
        dataset = MagicMock(dm_subset=MagicMock(df=dataframe))

        assert _dataset_needs_single_process_loading(dataset) is False

    def test_object_column_without_function_returns_false(self) -> None:
        """An Object column holding plain (picklable) values should not trigger the fallback."""
        import polars as pl

        from getitune.data.module import _dataset_needs_single_process_loading

        dataframe = pl.DataFrame({"meta": pl.Series([{"a": 1}], dtype=pl.Object)})
        dataset = MagicMock(dm_subset=MagicMock(df=dataframe))
    ...
    def test_empty_dataframe_returns_false(self) -> None:
        ...
        dataframe = pl.DataFrame({"labels": []})
        dataset = MagicMock(dm_subset=MagicMock(df=dataframe))

        assert _dataset_needs_single_process_loading(dataset) is False

    @pytest.mark.parametrize("bad_dataset", [MagicMock(dm_subset=None), object(), "not-a-dataset"])
    def test_defensive_on_non_dataframe_like_input(self, bad_dataset) -> None:
        from getitune.data.module import _dataset_needs_single_process_loading

        assert _dataset_needs_single_process_loading(bad_dataset) is False
