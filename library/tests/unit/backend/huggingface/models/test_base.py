...
def test_load_checkpoint_clears_threshold_when_checkpoint_has_none(tmp_path: Path) -> None:
    """Checkpoints saved before the F1 sweep must not carry another run's threshold."""
    model = HFMulticlassClsModel(_tiny_vit_config(), _label_info())
    ...
    # Stale value would silently filter the just-loaded weights; the engine
    # recomputes the threshold through the current validation sweep instead.
    assert model.best_confidence_threshold is None
