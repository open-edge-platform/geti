...
def test_find_one_and_update(request) -> None:
    # Arrange
    filter: dict[str, Any] = {"state": 0}
    update = {"state": 1}
    job_repo = SessionBasedSchedulerJobRepo()
    query_filter = job_repo.preliminary_query_match_filter(access_mode=QueryAccessMode.WRITE)
    ...
    collection.find_one_and_update.assert_called_once_with(
        filter=query_filter, update=update, upsert=False, session=None
    )


def test_delete_all(request) -> None:
    # Arrange
    filter: dict[str, Any] = {"state": 0}
    job_repo = SessionBasedSchedulerJobRepo()
    query_filter = job_repo.preliminary_query_match_filter(access_mode=QueryAccessMode.WRITE)
    ...
    # Act
    with patch.object(SessionBasedRepo, "delete_all") as mock_delete:
        job_repo.delete_all(extra_filter=filter)

    # Assert
    mock_delete.assert_called_once_with(extra_filter=filter)


def test_delete_all_empty_filter(request) -> None:
    # Arrange
    job_repo = SessionBasedSchedulerJobRepo()
