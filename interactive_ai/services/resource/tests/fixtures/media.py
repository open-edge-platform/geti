...
def fxt_solid_color_image_bytes():
    ...
    def _build_image(height: int = 50, width: int = 100, extension: str = "jpg", fill_value: int = 0) -> bytes:
        ...
        return io.BytesIO(blank_image).read()  # type: ignore[arg-type]

    yield _build_image


@pytest.fixture
def fxt_image_entity_factory(fxt_mongo_id, fxt_dataset_storage):
    def _build_image(index: int = 1, name: str = "dummy image"):
        return Image(
            name=name,
            uploader_id="",
            id=fxt_mongo_id(index),
            height=32,
            width=32,
            size=100,
...
@pytest.fixture
def fxt_video_entity(fxt_mongo_id):
    yield Video(
        name="dummy_video",
        id=fxt_mongo_id(2),
...
@pytest.fixture
def fxt_numpy_image_factory():
    def _build_numpy(image_width: int, image_height: int) -> np.ndarray:
...
def fxt_unannotated_video_factory(
    ...
    def _video_factory(
        ...
        for _i in range(number_of_frames):
            image = fxt_numpy_image_factory(image_width=width, image_height=height)
