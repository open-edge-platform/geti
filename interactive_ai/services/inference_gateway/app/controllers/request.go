...
type RequestHandler interface {
	...
	) (*entities.PredictionRequestData, error)
	NewBatchPredictionRequest(
		c *gin.Context,
		r *entities.InferenceRequest,
		entityID sdkentities.ID,
...
func (ic *RequestHandlerImpl) NewPredictionRequest(
	c *gin.Context,
	r *entities.InferenceRequest,
	entityID sdkentities.ID,
) (*entities.PredictionRequestData, error) {
	...
	if entityID.String() != "active" {
		if err = entityID.IsValid(); err != nil {
			return nil, fmt.Errorf("model ID is not valid: %w", err)
		}
	}
...
func (ic *RequestHandlerImpl) GetMedia(
	c *gin.Context,
	r *entities.InferenceRequest,
) (*bytes.Buffer, *entities.MediaInfo, error) {
	...
	// Try to get file from form data first
	header, formErr := c.FormFile("file")
	if formErr == nil {
		return processFormFile(ctx, header)
	}

	// No file in form, try to get media identifiers from JSON body
	var mediaRequestInfo MediaInfoJSON
	if err := c.ShouldBindBodyWithJSON(&mediaRequestInfo); err != nil {
		return nil, nil, err
	}

	// Parse media identifiers
	...
	// Get buffer based on provided IDs
	var err error
	buf, err = getMediaBuffer(ctx, ic, r, imageID, videoID, datasetID, frameIndex)
	if err != nil {
		return nil, nil, err
	}
...
func processFormFile(ctx context.Context, header *multipart.FileHeader) (*bytes.Buffer, *entities.MediaInfo, error) {
	...
	// Return with empty mediaInfo since we're using file data
	return buf, &entities.MediaInfo{}, nil
}

// Helper function to get media buffer based on IDs.
func getMediaBuffer(
	ctx context.Context,
	ic *RequestHandlerImpl,
	r *entities.InferenceRequest,
	imageID, videoID, datasetID sdkentities.ID,
	frameIndex int,
) (*bytes.Buffer, error) {
	switch {
	...
	default:
		return nil, errors.New("no file data, image or video id found in request")
	}
}

// NewBatchPredictionRequest gathers all required data (media, roi, cache info) for handling the prediction request.
func (ic *RequestHandlerImpl) NewBatchPredictionRequest(
	c *gin.Context,
	r *entities.InferenceRequest,
	entityID sdkentities.ID,
) (*entities.BatchPredictionRequestData, error) {
