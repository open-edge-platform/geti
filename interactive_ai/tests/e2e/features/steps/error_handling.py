...
def step_then_request_rejected(context: Context) -> None:
    ...
    if not hasattr(context, "exception"):
        assert False, (
            "The previous step did not raise an error, or it wasn't captured into the context"
        )

    if not isinstance(context.exception, ApiException):
        assert False, (
            "An error occurred during the previous step, but it does not originate from the API request"
        )
