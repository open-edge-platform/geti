...
def step_then_a_model_exists(context: Context, expected_model_type: str):
    ...
    matching_model_groups = [
        model_group
        for model_group in model_groups.model_groups
        if model_group.model_template_id == expected_model_type
    ]

    assert len(matching_model_groups) == 1, (
        f"There must be exactly one model group with the expected architecture '{expected_model_type}', found {len(matching_model_groups)} model groups."
    )
    ...
    models = sorted(
        context.model_storage_info.models, key=lambda model: model.version, reverse=True
    )

    assert len(matching_model_groups) > 0, (
        "There must be at least one model in the model group."
    )
