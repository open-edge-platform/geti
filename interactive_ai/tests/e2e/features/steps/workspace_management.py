...
if TYPE_CHECKING:
    from geti_client import (
        OrganizationsApi,
        PersonalAccessTokenOrganization200Response,
        WorkspacesApi,
    )
...
def step_given_workspace(context: Context) -> None:
    ...
    pat_org_response: PersonalAccessTokenOrganization200Response = (
        org_api.personal_access_token_organization()
    )
    ...
    if len(ws_response.workspaces) == 0:
        raise AssertionError(
            f"No workspaces found in organization {context.organization_id}"
        )
    if len(ws_response.workspaces) > 1:
        logger.warning(
            f"Multiple workspaces found in organization {context.organization_id}. Using the first one."
        )
