...
class DatasetHelpers:
    ...
    @staticmethod
    @unified_tracing
    def construct_fully_annotated_anomalous_dataset(
        project: Project,
        dataset_storage: DatasetStorage,
        task_node_id: ID,
        create_new_annotations: bool = False,
        ...
        """
        Construct dataset only containing fully annotated anomalous media
        ...
        :return: evaluation dataset from all annotated media in dataset storage
        """
        ann_scene_repo = AnnotationSceneStateRepo(dataset_storage.identifier)
        all_states = ann_scene_repo.get_all_by_state_for_task(
            matchable_annotation_states_per_task={task_node_id: [AnnotationState.ANNOTATED]}
        )
        items = []
        for annotation_state in all_states:
        ...
        dataset = Dataset(id=DatasetRepo.generate_id(), items=items)
