...
from iai_core_py.configuration.enums.config_element_type import ConfigElementType, ElementCategory
...
def construct_attrib_from_dict(dict_object: dict | DictConfig) -> ExposureTypeVar:  # type: ignore[type-var]
...
def construct_ui_rules_from_dict(ui_exposure_settings: dict | DictConfig) -> UIRules:
    ...
    for rule in rules_dict:
        ...
        if rule_type == RuleElementMapping.UI_RULES.name:
            ...
            raise ValueError(
                f"Invalid UI exposure settings passed to parser. Configuration contains invalid rule type: {rule_type}"
            )
...
def create_default_configurable_enum_from_dict(parameter_dict: dict | DictConfig) -> dict:
...
def gather_parameter_arguments_and_values_from_dict(config_dict_section: dict | DictConfig) -> dict:
    ...
    for key, value in config_dict_section.items():
        ...
        if isinstance(value, DictConfig | dict):
            ...
            for deprecated_key in metadata_keys.deprecated_keys():
                parameter_dict.pop(deprecated_key, None)
            ...
            if parameter_type == str(ConfigElementType.SELECTABLE):
                ...
                raise ValueError(
                    f"No type was specified for the configurable parameter or parameter group named '{key}'"  # type: ignore[str-bytes-safe]
                )
...
def create_parameter_group(config_dict_section: dict | DictConfig) -> ParameterGroupTypeVar:  # type: ignore[type-var]
...
def create_nested_parameter_group(config_dict_section: dict | DictConfig) -> ParameterGroup:
    """Creates a parameter group object out of a config_dict_section.
...
def contains_parameter_groups(config_dict: dict | DictConfig) -> list[str]:
...
def from_dict_attr(config_dict: dict | DictConfig) -> ConfigurableParameters:
    ...
    :return: ConfigurableParameters: ParameterGroup object constructed according to config_dict
    """
...
def create(input_config: str | DictConfig | dict) -> ConfigurableParameters:
