...
@pytest.fixture(scope="function")
def project_with_data(request, sample_project):
...
def sample_segmentation_model(project, dataset):
    ...
    segmentation_model = Model(
        ...
        training_framework=TrainingFramework(type=TrainingFrameworkType.OTX, version="1.0.0"),
...
def triangle_dataset(dataset: Dataset, labels: list[Label]):
    triangle_label_id = next(label.id_ for label in labels if label.name == "triangle")
    return get_figures_dataset(dataset, triangle_label_id)


def circle_dataset(dataset: Dataset, labels: list[Label]):
    circle_label_id = next(label.id_ for label in labels if label.name == "ellipse")
